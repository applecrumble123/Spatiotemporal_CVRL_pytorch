...
""" --------- Loss function ----------- """
def JVS_contrastive_loss(output1, output2):
    ...
    pos = torch.exp(((2 * (output1 * output2).sum(dim=1)) / (
                (output1 * output1).sum(dim=1) + (output2 * output2).sum(dim=1))))
    # print('pos: ', pos)
    # print(pos.size())
...
class CVLR(object):
    ...
    def __init__(self):
        ...
        self.writer = SummaryWriter(log_dir=config.TENSORBOARD_JVS_AND_CL_NO_TEMP)
    ...
    def _step(self, model, xis, xjs, n_iter):
        ...
        if len(zis) == 128 and len(zjs) == 128:
            ...
            loss = JVS_contrastive_loss(zis, zjs)
